...
def get_near_far(ray_o, ray_d, bounds):
    ...
    Returns:
        ...
        Here near is the depth in actual physical space
    """
...
class Base(nn.Module):
    ...
    def sample_pdf(self, near, far, ray_o, ray_d, split):
        ...
        raw_output = self.calculate_density(pts)
        # resample
    ...
    def calculate_density_from_ray(self, ray_o, ray_d, near, far, split):
    ...
    #    viewdirs = ray_d / torch.norm(ray_d, keepdim=True, dim=-1)
    #    viewdirs_ = viewdirs.expand(-1, pts.shape[1], -1)
    #    raw_output = self.calculate_density(pts, viewdirs_)
    #    return z_vals, pts, raw_output
