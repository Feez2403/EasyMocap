...
class Nerf(Base):
    ...
    def calculate_density(self, wpts, latents={}, **kwargs):
        # Linear mode
        # wpts: (..., 3) => (..., 63)
        # return: (..., 1)
        ...
        # density
        raw_alpha = alpha
        # remap to a value between 0 and 1
        if self.use_occupancy:
            occupancy = self.act_alpha(alpha)
...
if __name__ == "__main__":
    class SampleArgs:
        def __init__(self, method, N_samples) -> None:
            self.method = method
    ...
    cfg = {
        ...
        'use_occupancy': True,
        'linear_func': 'Linear',
        'sample_args': SampleArgs('uniform', 32),
    }
    network = Nerf(**cfg)
    pts = torch.rand((1, 4, 4, 64, 3))
    alpha = network.calculate_density(pts)
    print([alpha[key].shape for key in alpha.keys()]) # [(1, 4, 4, 64, 1), (1, 4, 4, 64, 1)]