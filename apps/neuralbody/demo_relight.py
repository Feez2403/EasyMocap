...
def neuralbody_train(data, config, mode, exp=None):
    ...
    cfg_name = join(outdir, 'config.yml')
    cmd =  f'python apps/neuralbody/train_pl_relight.py --cfg {cfg_name} gpus {args.gpus} distributed True exp {exp}'
    ...
    if args.eval or args.demo or args.test or args.trainvis or args.canonical or args.poses is not None:
        ...
        if len(demolists) == 0:
            ...
            newest = demolists[-1]
            for key in ['rgb_map', 'acc_map', 'feat_map', 'normal_map', 'depth_map', 'albedo_map', 'brdf_map']:
